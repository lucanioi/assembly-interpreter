describe Assembly::Instructions::Sub do
  subject(:sub_instruction) { described_class.new(target_register, source_register) }
  ...
  describe '#execute' do
    it 'adds the values together' do
      ...
      expect(program.get_register(target_register)).to eq 12
    end
    ...
    context 'when source register is just a raw-value integer' do
      ...
      it 'copies the integer into the register' do
        ...
        expect(program.get_register(target_register)).to eq 0
      end
    end
